...
def get_data_loaders(batch_size, data_dir, coeff_count):
    ...
    dataloaders = {x: DataLoader(train_data, batch_size=batch_size,
...
def print_test_loss(model, data_dir, coeff_count, device):
    test_loader = DataLoader(TestSet(f'{data_dir}/', coeff_count))
    coeff_index = coeff_count + 2  # 2 for angles
    coeff_index_2 = 2*coeff_count + 2  # 2 for angles
    theta_loss = 0.
    phi_loss = 0.
    shape_coeff_loss = 0.
