...
def generating_cumulative_error_plots(method_error_fnames: list, method_identifiers: list, out_fname : str):
    ...
    log_file = open(f'{out_fname}.txt', 'w')
    for i in range(len(method_identifiers)):
        log_file.write('%s - median: %f, mean: %f, std: %f \n' % (method_identifiers[i], np.median(np.hstack(method_errors[i])), np.mean(np.hstack(method_errors[i])), np.std(np.hstack(method_errors[i]))))
        telegram_logger(out_fname)
        telegram_logger('%s - median: %f, mean: %f, std: %f' % (method_identifiers[i], np.median(np.hstack(method_errors[i])), np.mean(np.hstack(method_errors[i])), np.std(np.hstack(method_errors[i]))))
    log_file.close()
    cumulative_errors = []
...
if __name__ == '__main__':
    # for base_path in glob('./predictions/bfm-data/bfm-code-new/outs/output-*/*/3D/results/'):
    # for base_path in glob('./predictions/*/*/3D/results/'):
    for base_path in glob('./predictions/camera-all-semi-supervised-bfm/outs/output*/*/*/supervised/*/*/3D/results/'):
        if os.path.exists(f'{base_path}semi-supervised-cuke-errors.txt'):
            continue
