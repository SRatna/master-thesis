...
class NewBranch(nn.Module):
    def __init__(self, input_size, coeff_count, exp_count, bfm_model, cam_params, is_poses=False):
        super(NewBranch, self).__init__()
        self.bfm_model = bfm_model
        self.cam_params = cam_params
        self.is_poses = is_poses
        if is_poses: # for backward compatibility as old model has fc_poses: default false
            self.fc_poses = nn.Linear(input_size, cam_params)
        else: self.fc_camera_params = nn.Linear(input_size, cam_params)
    ...
    def forward(self, inputs):
        ...
        x = x.squeeze(2)
        if self.is_poses:
            out_camera_params = self.fc_poses(torch.relu(x))
        else: out_camera_params = self.fc_camera_params(torch.relu(x))
