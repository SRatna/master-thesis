...
if __name__ == '__main__':
    ...
    # for predicted_mesh_folder in glob('./predictions/bfm-data/bfm-code-new/outs/output-*/*/3D/'):
    # for predicted_mesh_folder in glob('./predictions/*/*/3D/'):
    for predicted_mesh_folder in glob('./predictions/camera-all-semi-supervised-bfm/outs/output-*/*/*/supervised/*/*/3D/'):
        if os.path.exists(f'{predicted_mesh_folder}results/'):
            # make sure error file has content (no content means sthg wrong went last time during 3d reconstruction)
            if os.path.getsize(f'{predicted_mesh_folder}results/semi-supervised-cuke-errors.txt') > 0:
                # telegram_logger(f'{predicted_mesh_folder} already done')
                continue
        telegram_logger(f'{predicted_mesh_folder} started to compute mesh loss')
        # valid_challenges = ['', 'multiview_neutral', 'multiview_expressions', 'multiview_occlusions', 'selfie']
        valid_challenges = ['']
        for challenge in valid_challenges:
            metric_computation(dataset_folder, predicted_mesh_folder, challenge=challenge)
